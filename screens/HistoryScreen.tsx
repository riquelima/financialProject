...
const HistoryScreen: React.FC = () => {
  ...
  const sortedMonthYears = Object.keys(data || {}) // Ensure data is not null
    .sort((a, b) => {
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="space-y-4">
        {sortedMonthYears.map(monthYear => {
          ...
          const monthData = data[monthYear]; // data is confirmed to exist here
          
          if (!monthData) return null; // Should not happen if sortedMonthYears is from Object.keys(data)
            ...
            <div key={monthYear} className={`p-5 bg-${COLORS.cardBackground} rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div className="p-3 bg-slate-700 rounded-md">
                  ...
                  <p className={`font-medium text-${COLORS.textPrimary}`}>{formatCurrency(monthData.openingBalance, currencySymbol)}</p>
                </div>
                <div className="p-3 bg-slate-700 rounded-md">
                  <p className={`text-xs text-${COLORS.textSecondary}`}>Proventos</p>
                  <p className={`font-medium text-${COLORS.income}`}>{formatCurrency(summary.totalIncome, currencySymbol)}</p>
                </div>
                <div className="p-3 bg-slate-700 rounded-md">
                  <p className={`text-xs text-${COLORS.textSecondary}`}>Débitos</p>
                  <p className={`font-medium text-${COLORS.expense}`}>{formatCurrency(summary.totalExpenses, currencySymbol)}</p>
                </div>
                <div className={`p-3 rounded-md ${summary.netSavings >= 0 ? 'bg-emerald-800/50' : 'bg-red-800/50'}`}>
                  ...
                  <p className={`font-semibold text-lg ${summary.netSavings >= 0 ? `text-${COLORS.income}` : `text-${COLORS.expense}`}`}>{formatCurrency(summary.netSavings, currencySymbol)}</p>
                </div>
                 <div className="p-3 bg-slate-700 rounded-md sm:col-span-2 md:col-span-4">
                  ...
                  <p className={`font-semibold text-xl text-${COLORS.textPrimary}`}>{formatCurrency(summary.accountBalance, currencySymbol)}</p>
                </div>
              </div>
               <button
